...
async fn simple_run_config(config: &config::ConfigWithDefault, init: &str, commands: &str) -> String {
    let syntax_parent = accepted::syntax::SyntaxParent::default();
    let mut buf: Buffer<RopeyCoreBuffer> = Buffer::new(&syntax_parent, &config);
    buf.core.set_string(init.into(), true);
    let mut buffer = BufferMode::new(buf);
...
async fn test_simples() {
    ...
    assert_eq!(simple_run("123", "llllllIHello World").await, "Hello World123");
    assert_eq!(simple_run("    123", "lllllllllllllllllllIHello World").await, "    Hello World123");
    ...
    assert_eq!(simple_run("((123 456 789)(qwe))(abc)", "da)").await, "(abc)");
    ...
    // 0, $
    assert_eq!(simple_run("123 456 789", "ww0iabc ").await, "abc 123 456 789");
    ...
    // Run
    // Not crash
    assert_eq!(simple_run("#!/bin/sh\n\necho Hello", " tgG").await, "#!/bin/sh\n\necho Hello");
