use crate::ewin_core::_cfg::keys::{KeyCmd, Keybind};
use crate::prompt::prompt::choice::*;
use crate::{
    ewin_core::{colors::*, def::*, global::*, log::Log, util::*},
    model::*,
...
use ewin_core::model::{CurDirection, Encode, EvtActType, HeaderFile};
...
impl Prompt___ {
    ...
    pub fn enc_nl(term: &mut Terminal) {
        ...
        term.curt().prom.cont_1 = PromptCont::new_edit_type(term.curt().prom.keycmd.clone(), PromptContPosi::First).get_enc_nl(term);
        term.curt().prom.cont_2 = PromptCont::new_edit_type(term.curt().prom.keycmd.clone(), PromptContPosi::Second).get_enc_nl(term);
        term.curt().prom.cont_3 = PromptCont::new_edit_type(term.curt().prom.keycmd.clone(), PromptContPosi::Third).get_enc_nl(term);
        term.curt().prom.cont_4 = PromptCont::new_edit_type(term.curt().prom.keycmd.clone(), PromptContPosi::Fourth).get_enc_nl(term);
    }

    pub fn draw_enc_nl(&self, str_vec: &mut Vec<String>) {
        Log::debug_key("draw_open_file");

        Prompt___::set_draw_vec(str_vec, self.cont_1.buf_desc_row_posi, &self.cont_1.buf_desc.clone());
        Prompt___::draw_choice_enc_nl(self, str_vec, &self.cont_1);

        Prompt___::set_draw_vec(str_vec, self.cont_2.buf_desc_row_posi, &self.cont_2.buf_desc.clone());
        Prompt___::draw_choice_enc_nl(self, str_vec, &self.cont_2);
        ...
        Prompt___::set_draw_vec(str_vec, self.cont_3.buf_desc_row_posi, &cont_3_buf_desc);
        Prompt___::set_draw_vec(str_vec, self.cont_4.buf_desc_row_posi, &cont_4_buf_desc);

        if is_file_reload {
            Prompt___::set_draw_vec(str_vec, self.cont_3.buf_row_posi, &"".to_string());
            Prompt___::set_draw_vec(str_vec, self.cont_4.buf_row_posi, &"".to_string());
        } else {
            Prompt___::draw_choice_enc_nl(self, str_vec, &self.cont_3);
            Prompt___::draw_choice_enc_nl(self, str_vec, &self.cont_4);
        }
    }
    ...
    fn draw_choice_enc_nl(prom: &Prompt___, str_vec: &mut Vec<String>, prom_cont: &PromptCont) {
