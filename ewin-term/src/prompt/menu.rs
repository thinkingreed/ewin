...
use ewin_core::model::CurDirection;
        ...
        match term.curt().prom.keycmd {
            ...
            KeyCmd::MouseDownLeft(y, x) => {
                if term.curt().prom.left_down_choice_menu(y as u16, x as u16) {
                    return Prompt___::confirm_menu(term, true);
                }
            ...
            KeyCmd::ConfirmPrompt => {
                ...
                return Prompt___::confirm_menu(term, false);
            }
...
impl Prompt___ {
    pub fn menu(term: &mut Terminal) {
        ...
        let mut cont_1 = PromptCont::new_edit_type(term.curt().prom.keycmd.clone(), PromptContPosi::First);
        cont_1.set_menu(term);
        term.curt().prom.cont_1 = cont_1;
        let mut cont_2 = PromptCont::new_edit_type(term.curt().prom.keycmd.clone(), PromptContPosi::Second);
        cont_2.set_menu(term);
        term.curt().prom.cont_2 = cont_2;
        let mut cont_3 = PromptCont::new_edit_type(term.curt().prom.keycmd.clone(), PromptContPosi::Third);
    ...
    pub fn draw_menu(&self, str_vec: &mut Vec<String>) {
        ...
        Prompt___::set_draw_vec(str_vec, self.cont_3.buf_desc_row_posi, &self.cont_3.buf_desc.clone());
