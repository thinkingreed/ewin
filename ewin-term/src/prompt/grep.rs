use ewin_core::model::{DrawType, EvtActType, HeaderFile};

use crate::{
    ewin_core::{_cfg::keys::*, colors::*, global::*, log::Log},
    model::*,
        ...
        match term.curt().prom.keycmd {
            ...
            KeyCmd::ConfirmPrompt => {
                ...
                if search_str.len() == 0 {
                    ...
                    Prompt___::set_grep_working(term);
...
impl Prompt___ {
    pub fn grep(term: &mut Terminal) {
        ...
        term.curt().prom.cont_1 = PromptCont::new_edit_type(term.curt().prom.keycmd.clone(), PromptContPosi::First).get_grep(&term.curt().prom);
        term.curt().prom.cont_2 = PromptCont::new_edit_type(term.curt().prom.keycmd.clone(), PromptContPosi::Second).get_grep(&term.curt().prom);
        term.curt().prom.cont_3 = PromptCont::new_edit_type(term.curt().prom.keycmd.clone(), PromptContPosi::Third).get_grep(&term.curt().prom);
    }

    pub fn draw_grep(&self, str_vec: &mut Vec<String>) {
        Prompt___::set_draw_vec(str_vec, self.cont_1.opt_row_posi, &self.get_serach_opt());
        Prompt___::set_draw_vec(str_vec, self.cont_1.buf_desc_row_posi, &self.cont_1.buf_desc.clone());
        Prompt___::set_draw_vec(str_vec, self.cont_1.buf_row_posi, &self.cont_1.get_draw_buf_str());
        Prompt___::set_draw_vec(str_vec, self.cont_2.buf_desc_row_posi, &self.cont_2.buf_desc);
        Prompt___::set_draw_vec(str_vec, self.cont_2.buf_row_posi, &self.cont_2.get_draw_buf_str());
        Prompt___::set_draw_vec(str_vec, self.cont_3.buf_desc_row_posi, &self.cont_3.buf_desc);
        Prompt___::set_draw_vec(str_vec, self.cont_3.buf_row_posi, &self.cont_3.get_draw_buf_str());
    }
}

impl PromptCont {
    pub fn get_grep(&mut self, prom: &Prompt___) -> PromptCont {
