...
impl PromBase {
    pub fn ctrl_cont(&mut self) -> ActType {
        ...
        // updown content
        if self.config.is_updown_valid {
    ...
    pub fn get_curt_cont_mut(&mut self) -> Option<&mut Box<dyn PromContPluginTrait>> {
    ...
    pub fn get_tgt_cont(&mut self, i: usize) -> Option<&mut Box<dyn PromContPluginTrait>> {
    ...
    pub fn set_curt_cont_from_y(&mut self, y: usize, x: usize) {
        if let Ok(p_cont) = self.get_curt_cont_mut().unwrap().downcast_mut::<PromContPulldown>() {
            if p_cont.pulldown.is_disp {
                if !p_cont.pulldown.widget.is_mouse_within_area(y, x) {
    ...
    fn get_next_cont_proc(&self, vec: Vec<Box<dyn PromContPluginTrait>>) -> Option<usize> {
