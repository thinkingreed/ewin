    ...
    pub fn scroll(&mut self) {
        ...
        if self.cur.y == 0 {
            ...
            match &self.e_cmd {
                ...
                E_Cmd::DelNextChar | E_Cmd::DelPrevChar | E_Cmd::Undo | E_Cmd::Redo if self.offset_y > self.cur.y => self.offset_y = self.cur.y - Editor::SCROLL_UP_DOWN_EXTRA,
                E_Cmd::GrepResult | E_Cmd::CursorDown | E_Cmd::CursorDownSelect | E_Cmd::MouseScrollDown | E_Cmd::MouseDragLeftDown(_, _) | E_Cmd::CursorFileEnd | E_Cmd::InsertStr(_) | E_Cmd::InsertLine | E_Cmd::FindNext | E_Cmd::DelNextChar | E_Cmd::DelPrevChar | E_Cmd::Undo | E_Cmd::Redo => {
                    if self.cur.y + Editor::SCROLL_UP_DOWN_EXTRA >= self.row_num {
                        // "self.buf.len_lines() - self.row_num" is For the last row
                        self.offset_y = max(self.offset_y, min(self.buf.len_lines() - self.row_num, self.cur.y + 1 + Editor::SCROLL_UP_DOWN_EXTRA - self.row_num));
                    }
                }
                E_Cmd::CursorUp | E_Cmd::MouseScrollUp | E_Cmd::MouseDragLeftUp(_, _) | E_Cmd::CursorUpSelect | E_Cmd::FindBack => {
                    ...
                    if self.offset_y >= Editor::SCROLL_UP_DOWN_EXTRA && self.cur.y == Editor::SCROLL_UP_DOWN_EXTRA + self.offset_y - 1 {
