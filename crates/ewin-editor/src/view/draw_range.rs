...
impl Editor {
    pub fn set_draw_range(&mut self) {
        ...
        // judgment redraw
        Log::debug("self.draw_range Before setting", &self.win_mgr.curt().draw_range);

        self.win_mgr.curt().draw_range = if matches!(self.cmd.cmd_type, CmdType::Resize(_, _))
      ...
      ||  (self.cmd.config.is_edit && self.is_enable_syntax_highlight)
             ...
             || self.search != self.search_org
        {
            ...
            if (self.win_mgr.curt().offset.y_org as isize - self.win_mgr.curt().offset.y as isize).unsigned_abs() > self.get_curt_row_len() {
                ...
                match self.cmd.cmd_type {
                    CmdType::CursorUp | CmdType::MouseScrollUp | CmdType::MouseDragLeftUp(_, _) | CmdType::CursorDown | CmdType::MouseScrollDown | CmdType::MouseDragLeftDown(_, _) if self.win_mgr.split_type != WindowSplitType::None => E_DrawRange::All,
        ...
        Log::debug("self.draw_range After setting", &self.win_mgr.curt().draw_range);
    }
    ...
    pub fn get_draw_parts(&mut self) -> DParts {
        ...
        if self.change_info.change_type != EditerChangeType::Edit {
            if let E_DrawRange::TargetRange(sy, ey) = self.win_mgr.curt().draw_range {
                self.change_info.restayle_row_set.append(&mut (sy..=ey).collect::<BTreeSet<usize>>());
            }
            if let E_DrawRange::All = self.win_mgr.curt().draw_range {
