...
impl Editor {
    pub fn set_draw_range(&mut self) {
      ...
      ||  (Editor::is_edit(&self.e_cmd, true) && self.is_enable_syntax_highlight)
            ...
            match &self.e_cmd {
                ...
                E_Cmd::InsertStr(str) => {
                    if self.is_enable_syntax_highlight || self.box_insert.mode == BoxInsertMode::Insert {
                        ...
                        E_DrawRange::After(self.cur_org.y)
                    } else {
                        E_DrawRange::TargetRange(self.cur.y, self.cur.y)
                    }
                }
                ...
                E_Cmd::AllSelect => {
                    self.change_info.restayle_row = self.get_row_in_screen();
                    E_DrawRange::All
                }
                ...
                E_Cmd::CancelModeAndSearchResult => {
                    if self.sel_org.is_selected() {
                        self.change_info.restayle_row = (self.sel_org.sy..=self.sel_org.ey).collect::<BTreeSet<usize>>();
                    }
        ...
        if self.change_info.change_type == EditerChangeType::None {
