...
impl MenuBar {
    pub fn ctrl_menubar(cmd_type: &CmdType) -> ActType {
        if let Some(mut menubar) = MenuBar::get_result() {
                    ...
                    if *y == menubar.view.y {
                        ...
                        if is_on_mouse {
                            ...
                            if menubar.is_menu_changed() {
                                return ActType::Draw(DrawParts::MenuBar);
                            }
                        }
    ...
    pub fn select_menulist(&mut self) -> ActType {
        ...
        if let Some((child, _)) = self.menulist.curt.get_curt_child() {
            ...
            return self.exec_menubar_func(&menubar_cont.menunm, &self.menulist.curt.get_curt_parent().unwrap().0.name, &child.name);
        } else if !self.menulist.curt.is_exist_child_curt_parent() {
            if let Some((parent, _)) = self.menulist.curt.get_curt_parent() {
                Log::debug("parent", &parent);
                return self.exec_menubar_func(&menubar_cont.menunm, &parent.name, "");
            }
        }
        return ActType::Cancel;
    }
    pub fn exec_menubar_func(&mut self, parent_name: &str, child_name: &str, grandchild_name: &str) -> ActType {
    ...
    pub fn init_menubar(&mut self, y: usize) {
        ...
        if self.menulist.curt.cont.x_area.0 != menu.area.start {
            ...
            let height = min(self.menulist.curt.cont.cont_vec.len(), get_term_size().1 - MENUBAR_HEIGHT);
