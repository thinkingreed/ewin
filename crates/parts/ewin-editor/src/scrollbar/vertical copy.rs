...
impl Editor {
    pub fn calc_editor_scrlbar_v(&mut self) {
        ...
        for vec_v in self.win_mgr.win_list.iter_mut() {
            for win in vec_v.iter_mut() {
                ...
                win.scrl_v.view.y = match self.cmd.cmd_type {
                    CmdType::MouseDownLeft(_, _) | CmdType::MouseDragLeftUp(_, _) | CmdType::MouseDragLeftDown(_, _) | CmdType::MouseDragLeftLeft(_, _) | CmdType::MouseDragLeftRight(_, _) if win.scrl_v.is_enable => win.scrl_v.view.y,
    ...
    pub fn set_scrlbar_v_posi(&mut self, y: usize) {
        ...
        if matches!(self.cmd.cmd_type, CmdType::MouseDownLeft(_, _)) {
            ...
            // Except on scrl_v
            if !(self.get_curt_row_posi() + self.win_mgr.curt().scrl_v.view.y <= y && y < self.get_curt_row_posi() + self.win_mgr.curt().scrl_v.view.y + self.win_mgr.curt().scrl_v.bar_len) {
                self.win_mgr.curt().scrl_v.view.y = if y + self.win_mgr.curt().scrl_v.bar_len > self.get_curt_row_posi() + self.get_curt_row_len() - 1 { self.get_curt_row_posi() + self.get_curt_row_len() - 1 - self.win_mgr.curt().scrl_v.bar_len } else { y - self.get_curt_row_posi() };
            } else {
                return;
            }
            // MouseDragLeftDown・MouseDragLeftUp
        } else if self.win_mgr.curt().scrl_v.is_enable {
            if matches!(self.cmd.cmd_type, CmdType::MouseDragLeftDown(_, _)) && self.get_curt_row_len() >= self.win_mgr.curt().scrl_v.view.y + self.win_mgr.curt().scrl_v.bar_len {
                self.win_mgr.curt().scrl_v.view.y = if self.win_mgr.curt().scrl_v.view.y + self.win_mgr.curt().scrl_v.bar_len >= self.get_curt_row_len() { self.win_mgr.curt().scrl_v.view.y } else { self.win_mgr.curt().scrl_v.view.y + 1 };
                ...
                self.win_mgr.curt().scrl_v.view.y = if self.win_mgr.curt().scrl_v.view.y == 0 { self.win_mgr.curt().scrl_v.view.y } else { self.win_mgr.curt().scrl_v.view.y - 1 };
            }
        }
