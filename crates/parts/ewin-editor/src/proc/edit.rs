...
impl Editor {
    ...
    pub fn edit_proc(&mut self, cmd: Cmd) -> ActType {
        ...
        let is_selected_org = self.win_mgr.curt_mut().sel.is_selected_width();
        let mut proc_del = Proc::default();

        // selected range delete
        Log::debug("self.sel.is_selected_width()", &self.win_mgr.curt_mut().sel.is_selected_width());
        if self.win_mgr.curt_mut().sel.is_selected_width() && cmd.config.is_edit {
            ...
            proc_del.cur_s = Cur { y: self.win_mgr.curt_mut().sel.sy, x: self.win_mgr.curt_mut().sel.sx, disp_x: self.win_mgr.curt_mut().sel.s_disp_x };
            proc_del.cur_e = self.win_mgr.curt_mut().cur;
            match self.win_mgr.curt_mut().sel.mode {
                SelMode::Normal => proc_del.str = self.buf.slice(self.win_mgr.curt_mut().sel.get_range()),
                SelMode::BoxSelect => self.set_box_sel(&mut proc_del),
            }
            proc_del.sel = self.win_mgr.curt_mut().sel;
        ...
        match cmd.cmd_type {
            ...
            _ => {
                ...
                proc.cur_s = self.win_mgr.curt_mut().cur;
        ...
        if cmd.cmd_type != CmdType::Undo && cmd.cmd_type != CmdType::Redo {
        ...
        let is_changed_org = State::get().curt_ref_state().editor.is_changed;
        State::get().curt_mut_state().editor.is_changed = true;
        if is_changed_org != State::get().curt_ref_state().editor.is_changed {
    ...
    pub fn slice_box_sel(&mut self) -> (String, Vec<(SelRange, String)>) {
        let (sy, ey) = if self.win_mgr.curt_mut().sel.is_selected() { (self.win_mgr.curt_mut().sel.sy, self.win_mgr.curt_mut().sel.ey) } else { (self.box_insert.vec.first().unwrap().0.sy, self.box_insert.vec.last().unwrap().0.sy) };
        ...
        Log::debug("sy", &sy);
        Log::debug("ey", &ey);
        Log::debug("self.sel.get_range()", &self.win_mgr.curt_mut().sel.get_range());

        for y in sy..=ey {
            let mut row_sel = self.win_mgr.curt_mut().sel.get_range();
            ...
            if row_sel.is_selected_width() {
                ...
                row_sel.sx = self.win_mgr.curt_mut().cur.x;
                row_sel.ex = self.win_mgr.curt_mut().cur.x;
                row_sel.s_disp_x = self.win_mgr.curt_mut().cur.disp_x;
                row_sel.e_disp_x = self.win_mgr.curt_mut().cur.disp_x;
            }
            row_sel.sy = y;
            row_sel.ey = y;
            ...
            string.push_str(&NL::get_nl(&State::get().curt_ref_state().file.nl));
        }
    ...
    pub fn get_disp_x_range_string(&mut self, vec: Vec<char>) -> (String, usize, usize) {
        ...
        for (idx, c) in vec.iter().enumerate() {
            ...
            let width_org = width;
            let c_len = get_char_width(c, width + self.win_mgr.curt_mut().offset.disp_x);
            width += c_len;
            cur_x_e += 1;
    ...
    pub fn set_box_sel_vec(&mut self, ep_del: &Proc, ep: &mut Proc) {
        if self.box_insert.mode == BoxInsertMode::Insert || self.win_mgr.curt_mut().sel.mode == SelMode::BoxSelect {
        ...
        if self.box_insert.mode == BoxInsertMode::Insert {
            ...
            if str.is_empty() {
                ...
                if self.get_clipboard(proc) {
                    ...
                    self.win_mgr.curt_mut().cur.y = self.box_insert.vec.first().unwrap().0.sy;
                }
    ...
    pub fn set_box_str_vec(&mut self, ins_str: &str, ep: &mut Proc) {
        for i in 0..=ep.box_sel_vec.len() - 1 {
            ...
            ep.box_sel_vec[i].0.s_disp_x = self.win_mgr.curt_mut().cur.disp_x;
