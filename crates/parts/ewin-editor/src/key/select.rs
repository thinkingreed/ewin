...
impl Editor {
    pub fn all_select(&mut self) {
        ...
        self.win_mgr.curt_mut().sel.set_s(0, 0, 0);
        let (cur_x, width) = get_row_cur_x_disp_x(&self.buf.char_vec_row(self.buf.len_rows() - 1)[..], 0, false);
        self.win_mgr.curt_mut().sel.set_e(self.buf.len_rows() - 1, cur_x, width);
    }

    pub fn cut(&mut self, ep: Proc) -> ActType {
        Log::debug_key("cut");
        Log::debug("self.sel.is_selected()", &self.win_mgr.curt_mut().sel.is_selected());
    ...
    pub fn copy(&mut self) -> ActType {
        ...
        let copy_str = match self.win_mgr.curt_mut().sel.mode {
            SelMode::Normal => self.buf.slice(self.win_mgr.curt_mut().sel.get_range()),
            //
            SelMode::BoxSelect => {
