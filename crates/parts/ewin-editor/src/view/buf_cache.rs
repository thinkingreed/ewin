...
use ewin_key::util::*;
use ewin_state::tabs::*;
use ewin_view::{char_style::*, model::*, sel_range::*};
use std::cmp::{max, min};
...
impl EditorDraw {
    ...
    pub fn draw_cache(&mut self, editor: &Editor, win: &Window) {
        ...
        Log::debug("editor.d_range", &editor.draw_range);

        match editor.draw_range {
            E_DrawRange::After(sy) => {
                self.sy = sy;
                self.ey = min(win.offset.y + win.height() - 1, editor.buf.len_rows() - 1);
            }
            ...
            E_DrawRange::Init | E_DrawRange::All | E_DrawRange::WinOnlyAll | E_DrawRange::Targetpoint => {
        ...
        match editor.draw_range {
            E_DrawRange::Init | E_DrawRange::TargetRange(_, _) | E_DrawRange::After(_) | E_DrawRange::All | E_DrawRange::WinOnlyAll | E_DrawRange::Targetpoint | E_DrawRange::ScrollDown(_, _) | E_DrawRange::ScrollUp(_, _) => self.set_draw_regions(editor, win),
            E_DrawRange::Not | E_DrawRange::MoveCur => {}
        }
    }
    ...
    fn set_draw_regions(&mut self, editor: &Editor, win: &Window) {
        ...
        match &editor.draw_range {
            E_DrawRange::Init | E_DrawRange::All | E_DrawRange::WinOnlyAll | E_DrawRange::TargetRange(_, _) | E_DrawRange::ScrollDown(_, _) | E_DrawRange::ScrollUp(_, _) => change_row_vec = (self.sy..=self.ey).collect::<Vec<usize>>(),
            ...
            if !self.cells_all[y].is_empty() && self.cells_all[y].len() >= ex {
                ...
                if self.style_vecs[y].is_empty() {
                    if self.syntax_state_vec.is_empty() {
                        ...
                        parse = ParseState::new(&CfgSyntax::get().syntax.syntax_set.find_syntax_by_extension(&h_file.file.ext).cloned().unwrap());
                    } else {
