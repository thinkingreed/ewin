...
impl DialogContFileProp {
    pub fn get_kvs_vec(&mut self, key_vec: Vec<String>, value_max_width: usize) -> Vec<(String, Vec<String>)> {
        let file = &State::get().tabs.vec[self.idx].file.clone(); //  FileBar::get().tgt_h_file(self.idx).file.clone();
        let mut kvs_vec = vec![];
        ...
        for key in key_vec {
            if key.contains(&Lang::get().place) {
                ...
                let path_vec = if file.fullpath.is_empty() { vec![unsettled.clone()] } else { split_tgt_str_width(&file.fullpath, &[MAIN_SEPARATOR], value_max_width) };
                ...
                if file.fullpath.is_empty() {
                    ...
                    if !byte_str.contains(BYTE_KEY) {
                        ...
                        let _ = write!(byte_str, "({} bytes)", file.len.to_formatted_string(&Locale::en));
                    }
                }
