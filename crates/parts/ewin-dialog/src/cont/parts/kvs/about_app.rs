    ...
    pub fn get_kvs_vec(&mut self, key_vec: Vec<String>, value_max_width: usize) -> Vec<(String, Vec<String>)> {
        ...
        for key in key_vec {
            if key.contains(&DialogContAboutApp::VERSION) {
                ...
                let mut repository = split_tgt_width(&env!("CARGO_PKG_REPOSITORY"), &['/'], value_max_width);
                let wiki = split_tgt_width(&format!("{}{}", env!("CARGO_PKG_REPOSITORY"), "/wiki"), &['/'], value_max_width);
