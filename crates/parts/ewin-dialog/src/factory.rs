...
use ewin_const::{models::dialog::*, term::*};
...
impl Dialog {
    ...
    pub fn to_dialog(cont_type: DialogContType) -> Dialog {
        let mut dialog = match cont_type {
            DialogContType::FileProp(idx) => Dialog {
                is_show: true,
                cont: Box::new(DialogContFileProp { base: DialogContBase { title: format!("{}{}", Lang::get().file, Lang::get().file_property), cfg: DialogContCfg { max_width: (get_term_size().0 as f32 * 0.7) as usize, ..DialogContCfg::default() }, ..DialogContBase::default() }, idx }),
