...
use ewin_menulist::{core::*, parts::ctx_menu::*};
use ewin_state::tabs::*;
use std::{cmp::min, io::stdout};

impl EvtAct {
    pub fn ctrl_ctx_menu(term: &mut Term) -> ActType {
        Log::debug_key("ctrl_ctx_menu");

        match term.ctx_menu.cmd.cmd_type {
            CmdType::MouseDownLeft(y, x) => {
                if term.ctx_menu.menulist.is_mouse_within_area(y, x) {
            ...
            CmdType::MouseMove(y, x) => {
                if term.ctx_menu.menulist.is_mouse_within_area(y, x) {
                    let child_cont_org = &term.ctx_menu.menulist.cont.cont_vec.get(term.ctx_menu.menulist.parent_sel_y).and_then(|cont| cont.1.clone());
                    term.ctx_menu.menulist.ctrl_mouse_move(y, x);

                    if !term.ctx_menu.menulist.is_menu_change() {
                        return ActType::Cancel;
                    }
                    let child_cont = &term.ctx_menu.menulist.cont.cont_vec.get(term.ctx_menu.menulist.parent_sel_y).and_then(|cont| cont.1.clone());

                    // Only parent meun move || Only child meun move
                    if child_cont_org.is_none() && child_cont.is_none() || term.ctx_menu.menulist.parent_sel_y == term.ctx_menu.menulist.parent_sel_y_org && term.ctx_menu.menulist.child_sel_y != USIZE_UNDEFINED {
                        Log::debug_key("11111111111111111111111111111111");
                        return ActType::Draw(DParts::CtxMenu);
                    } else {
                        return ActType::Draw(DParts::Absolute(term.ctx_menu.menulist.get_disp_range_y()));
                    }
                } else if term.ctx_menu.menulist.is_mouse_area_around(y, x) {
                    term.ctx_menu.menulist.clear_select_menu();
            ...
            CmdType::CursorDown | CmdType::CursorUp | CmdType::CursorRight | CmdType::CursorLeft => {
                match term.ctx_menu.cmd.cmd_type {
                    CmdType::CursorDown => term.ctx_menu.menulist.cur_move(Direction::Down),
                    CmdType::CursorUp => term.ctx_menu.menulist.cur_move(Direction::Up),
                    CmdType::CursorRight => term.ctx_menu.menulist.cur_move(Direction::Right),
                    CmdType::CursorLeft => term.ctx_menu.menulist.cur_move(Direction::Left),
                    _ => {}
                }
                if !term.ctx_menu.menulist.is_menu_change() {
                    return ActType::Cancel;
    ...
    pub fn select_ctx_menu(term: &mut Term) -> ActType {
        Log::debug_key("select_ctx_menu");
        if let Some((ctx_menu, _)) = term.ctx_menu.menulist.get_curt_child() {
            return EvtAct::check_ctx_menu_func(term, &term.ctx_menu.menulist.get_curt_parent().unwrap().0.name, &ctx_menu.name);
        } else if !term.ctx_menu.menulist.is_exist_child_curt_parent() {
            if let Some((parent_ctx_menu, _)) = term.ctx_menu.menulist.get_curt_parent() {
                return EvtAct::check_ctx_menu_func(term, &parent_ctx_menu.name, "");
            }
        }
        return ActType::Cancel;
    }
    pub fn check_ctx_menu_func(term: &mut Term, parent_name: &str, child_name: &str) -> ActType {
    ...
    pub fn exec_ctx_menu_func(term: &mut Term, name: &str) -> ActType {
    ...
    pub fn set_ctx_menu_curt_term_place(term: &mut Term, y: usize) {
        if term.fbar.row_posi == y {
            term.ctx_menu.menulist.cont = term.ctx_menu.ctx_menu_place_map[&TermPlace::HeaderBar].clone();
            ...
            term.ctx_menu.menulist.cont = term.ctx_menu.ctx_menu_place_map[&place_cond].clone();
        }
    }

    pub fn is_ctx_menu_displayed_area(term: &mut Term, y: usize, x: usize) -> bool {
        if y == term.fbar.row_posi {
            for h_file in Tabs::get().h_file_vec.iter() {
...
impl Term {
    pub fn init_ctx_menu(&mut self, y: usize, x: usize) {
        ...
        if EvtAct::is_ctx_menu_displayed_area(self, y, x) {
            ...
            let height = min(self.ctx_menu.menulist.cont.cont_vec.len(), Editor::get_disp_row_num());
            self.ctx_menu.menulist.set_parent_disp_area(y, x, height);
    ...
    pub fn is_ctx_menu_keys(&mut self, keys: &Keys) -> bool {
        if self.state.is_ctx_menu {
            let rtn = match keys {
                ...
                Keys::MouseDownLeft(y, x) => self.ctx_menu.menulist.is_mouse_within_area(*y as usize, *x as usize),
