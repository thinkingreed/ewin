...
impl EvtAct {
    pub fn ctrl_filebar(term: &mut Terminal) -> ActType {
        ...
        match &term.keycmd {
            KeyCmd::FileBar(h_cmd) => match &h_cmd {
                F_Cmd::MouseDownLeft(y, x) => {
                        ...
                        for (idx, h_file) in term.fbar.file_vec.iter().enumerate() {
                            ...
                            if h_file.close_area.0 <= x && x <= h_file.close_area.1 {
                                if term.curt().editor.state.is_changed {
                                    ...
                                    term.tab_idx = if idx == term.fbar.file_vec.len() - 1 { idx - 1 } else { idx };
                                    term.del_tab(idx);
                    ...
                    if term.fbar.plus_btn_area.0 <= x && x <= term.fbar.plus_btn_area.1 {
                        ...
                        return ActType::Draw(DParts::All);
                    } else if term.fbar.menu_btn_area.0 <= x && x <= term.fbar.menu_btn_area.1 {
                ...
                F_Cmd::MouseDragLeftRight(y, x) | F_Cmd::MouseDragLeftLeft(y, x) => {
                    let (x, _) = (*x as usize, *y as usize);
                    ...
                    if term.fbar.all_filenm_space_w >= x {
                        ...
                        for (idx, h_file) in term.fbar.file_vec.iter().enumerate() {
                            ...
                            if h_file.filenm_area.0 <= x && x <= h_file.filenm_area.1 {
                                ...
                                if matches!(h_cmd, F_Cmd::MouseDragLeftLeft(_, _)) {
                                    Log::debug("MouseDragLeftLeft change_range", &change_range);
                                    if h_file.filenm_area.0 + change_range >= x {
                                        inset_idx = idx;
                                    }
                                } else if matches!(h_cmd, &F_Cmd::MouseDragLeftRight(_, _)) {
                ...
                F_Cmd::MouseUpLeft(_, _) => {
                    term.fbar.state.is_dragging = false;
                    return ActType::Draw(DParts::FileBar);
                }
                F_Cmd::MouseDragLeftDown(_, _) | F_Cmd::MouseDragLeftUp(_, _) => return ActType::Cancel,
            },
