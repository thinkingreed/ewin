...
impl EvtAct {
    ...
    pub fn prompt_check_input_field(term: &mut Terminal) -> ActType {
        Log::debug_key("prompt_check_input_field");
        Log::debug("term.curt().state.grep.is_grep", &term.curt().state.grep.is_grep);
        ...
        match term.curt().prom.p_cmd {
            ...
            P_Cmd::InsertStr(_) => term.curt().prom_plugin.proc(),
        ...
        // draw Prompt
        if !state.is_move_row && !state.is_search && !state.grep.is_grep && !state.is_open_file {
    ...
    pub fn prompt_check_search_opt(term: &mut Terminal) -> Option<ActType> {
        Log::debug_key("prompt_check_search_opt");
        Log::debug("term.curt().state.grep.is_grep", &term.curt().state.grep.is_grep);
