...
impl EvtAct {
    pub fn close(editor: &mut Editor, mbar: &mut MsgBar, prom: &mut Prompt, help: &mut Help, sbar: &mut StatusBar) -> EvtActType {
        match editor.evt {
            Key(KeyEvent { code: Char(c), .. }) => {
                if c == 'y' {
                    // save成否判定
                    if editor.save(mbar, prom, help, sbar) {
...
impl Prompt {
    pub fn close(&mut self, editor: &mut Editor, mbar: &mut MsgBar, help: &mut Help, sbar: &mut StatusBar) -> bool {
        if self.is_change == true {
            self.save_confirm_str(editor, mbar, help, sbar);
            self.is_close_confirm = true;
            return false;
        };
        return true;
    }
    pub fn save_confirm_str(&mut self, editor: &mut Editor, mbar: &mut MsgBar, help: &mut Help, sbar: &mut StatusBar) {
        self.disp_row_num = 2;
