...
impl EvtAct {
        ...
        match editor.evt {
            Key(KeyEvent { modifiers: KeyModifiers::SHIFT, code }) => match code {
                F(4) => {
                    ...
                    EvtAct::exec_search_confirm(out, editor, mbar, prom, sbar, false);
            ...
            Key(KeyEvent { code, .. }) => match code {
                F(3) => {
                    Log::ep_s("search.F3");
                    if EvtAct::exec_search_confirm(out, editor, mbar, prom, sbar, true) {
                        return EvtActType::Next;
                    } else {
    ...
    fn exec_search_confirm<T: Write>(out: &mut T, editor: &mut Editor, mbar: &mut MsgBar, prom: &mut Prompt, sbar: &mut StatusBar, is_asc: bool) -> bool {
    ...
    pub fn exec_search_incremental<T: Write>(out: &mut T, editor: &mut Editor, mbar: &mut MsgBar, prom: &mut Prompt, sbar: &mut StatusBar) {
        ...
        editor.search.ranges = if editor.search.str.len() == 0 { vec![] } else { editor.get_search_ranges(&editor.search.str, 0, editor.buf.line_to_char(ey) - 1) };
