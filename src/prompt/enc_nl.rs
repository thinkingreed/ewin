    ...
    pub fn enc_nl(term: &mut Terminal) -> EvtActType {
        match term.curt().editor.evt {
            Mouse(M_Event { kind: M_Kind::Down(M_Btn::Left), column: x, row: y, .. }) => {
                let x = x as usize;
                term.tabs[term.idx].prom.left_down_choice_enc_nl(y, x);
...
impl Prompt {
    fn left_down_choice_enc_nl(&mut self, y: u16, x: usize) {
        match y {
    ...
    pub fn draw_cur_enc_nl(&self, str_vec: &mut Vec<String>) {
        match self.prom_cont_posi {
            ...
            PromptContPosi::Third => self.cont_3.draw_cur_promcont(str_vec),
            PromptContPosi::Fourth => self.cont_4.draw_cur_promcont(str_vec),
        };
    }
    ...
    pub fn tab_enc_nl(&mut self, is_asc: bool) {
        match self.prom_cont_posi {
            PromptContPosi::First => self.cont_1.choices.set_next_back_choice(is_asc),
            PromptContPosi::Second => {
                self.cont_2.choices.set_next_back_choice(is_asc);
                self.enter_enc_ctrl_bom();
            }
            ...
            for (item_idx, item) in vec.iter().enumerate() {
                ...
                match prom_cont.posi {
                    PromptContPosi::Third | PromptContPosi::Fourth => {
                        let item = prom.cont_1.choices.get_choice();
...
impl PromptCont {
    fn get_enc_nl(&mut self, term: &mut Terminal) -> PromptCont {
        ...
        match self.posi {
            PromptContPosi::First => {
                ...
                self.set_default_choice_enc_nl(h_file, &mut msg_vec);
            ...
            PromptContPosi::Second => {
                ...
                self.set_default_choice_enc_nl(h_file, &mut enc_vec);
            ...
            PromptContPosi::Third => {
                ...
                self.set_default_choice_enc_nl(h_file, &mut nl_vec);
            ...
            PromptContPosi::Fourth => {
                ...
                self.set_default_choice_enc_nl(h_file, &mut bom_vec);
    ...
    fn set_default_choice_enc_nl(&mut self, h_file: &HeaderFile, vec: &mut Vec<Vec<Choice>>) {
        let mut total_idx = 0;
        for v in vec {
            let mut row_width = 1;
