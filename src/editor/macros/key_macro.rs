...
impl Tab {
    pub fn record_key_macro_start(&mut self) {
        ...
        if self.state.key_macro_state.is_record {
            ...
            self.editor.macros.key_macro_vec = vec![];
        }
    }

    pub fn exec_macro_key<T: Write>(out: &mut T, term: &mut Terminal) {
        Log::debug("key_record_vec", &term.curt().editor.macros.key_macro_vec);

        if term.curt().editor.macros.key_macro_vec.len() > 0 {
            term.curt().state.key_macro_state.is_exec = true;

            let macro_vec = term.curt().editor.macros.key_macro_vec.clone();
