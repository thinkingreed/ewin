...
impl StatusBar {
    ...
    pub fn draw(&mut self, str_vec: &mut Vec<String>, editor: &mut Editor) {
        ...
        if file_str.len() == 0 {
            ...
            //   if self.filenm_disp_flg == false {
            self.filenm_disp_flg = true;
            //   }
        }
    ...
    pub fn draw_cur(&mut self, str_vec: &mut Vec<String>, editor: &mut Editor) {
        ...
        let (filenm_w, _) = self.get_areas_width(self.disp_col_num);
        let sber_str = format!("{}{}{}{}{}", cursor::Goto(filenm_w as u16, self.disp_row_posi as u16), clear::UntilNewline, Fg(Rgb(221, 72, 20)), cur_str, Fg(White).to_string(),);
