use crate::{_cfg::keys::*, colors::*, ctx_menu::init::*, def::*, global::*, log::Log, model::*, prompt::prompt::prompt::*, terminal::*, util::*};
...
impl CtxMenuGroup {
    ...
    pub fn exec_func(term: &mut Terminal, name: &str) {
        match &LANG_MAP[name] {
            ...
            s if s == &LANG.xml => {
                Editor::format(term, FmtType::XML);
                term.curt().editor.sel.clear();
            }
            s if s == &LANG.html => {
                Editor::format(term, FmtType::HTML);
                term.curt().editor.sel.clear();
            }
            s if s == &LANG.cut => {
