...
impl Help {
    ...
    pub fn draw(&mut self, str_vec: &mut Vec<String>) {
        ...
        if self.key_bind_vec.is_empty() {
            // let mut key_bind_vec: Vec<Vec<KeyBind>> = vec![];
            let mut vec: Vec<KeyBind> = vec![];

            // 1st line
            self.set_key_bind(&mut vec, KEY_CLOSE, &LANG.end);
            self.set_key_bind(&mut vec, KEY_SAVE, &LANG.save);
            self.set_key_bind(&mut vec, KEY_COPY, &LANG.copy);
            self.set_key_bind(&mut vec, KEY_PASTE, &LANG.paste);
            self.key_bind_vec.push(vec.clone());
            vec.clear();
            // 2nd line
            self.set_key_bind(&mut vec, KEY_UNDO, &LANG.undo);
            self.set_key_bind(&mut vec, KEY_REDO, &LANG.redo);
            self.set_key_bind(&mut vec, KEY_SEARCH, &LANG.search);
            self.set_key_bind(&mut vec, KEY_REPLACE, &LANG.replace);
            self.key_bind_vec.push(vec.clone());
            vec.clear();
            // 3rd line
            self.set_key_bind(&mut vec, KEY_CUT, &LANG.cut);
            self.set_key_bind(&mut vec, KEY_GREP, &LANG.grep);
            self.set_key_bind(&mut vec, KEY_ALL_SELECT, &LANG.all_select);
            self.key_bind_vec.push(vec.clone());
            vec.clear();
            // 4th line
            self.set_key_bind_ex(&mut vec, KEY_SELECT, &LANG.range_select, 21, 17);
            self.set_key_bind(&mut vec, KEY_RECORD_START, &LANG.key_record_start);
            self.set_key_bind(&mut vec, KEY_RECORD_STOP, &LANG.key_record_stop);
            self.key_bind_vec.push(vec.clone());
            vec.clear();
            // 5th line
